"""Tests for apf.generations module."""
import sys
sys.path.insert(0, 'src')


def test_generations_all_pass():
    from apf.bank import run_all
    results = run_all(modules=['generations'], verbose=False)
...
def test_generations_count():
    ...
    mods = list_modules()
    assert len(mods['generations']) > 0, "Module has no theorems"


def test_generations_result_format():
    ...
    for name in _MODULE_MAP.get('generations', []):
        r = REGISTRY[name]()
...
if __name__ == '__main__':
    test_generations_all_pass()
    test_generations_count()
    test_generations_result_format()
    print(f"  generations: all tests passed")
