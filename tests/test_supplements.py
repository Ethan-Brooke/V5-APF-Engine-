"""Tests for apf.supplements module."""
import sys
sys.path.insert(0, 'src')


def test_supplements_all_pass():
    from apf.bank import run_all
    results = run_all(modules=['supplements'], verbose=False)
    failures = [n for n, r in results.items() if not r.get('passed')]
    assert not failures, f"Failed: {failures}"


def test_supplements_count():
    from apf.bank import list_modules
    mods = list_modules()
    assert len(mods['supplements']) > 0, "Module has no theorems"


def test_supplements_result_format():
    ...
    for name in _MODULE_MAP.get('supplements', []):
        r = REGISTRY[name]()
...
if __name__ == '__main__':
    test_supplements_all_pass()
    test_supplements_count()
    test_supplements_result_format()
    print(f"  supplements: all tests passed")
