"""Tests for apf.validation module."""
import sys
...
def test_validation_all_pass():
    ...
    results = run_all(modules=['validation'], verbose=False)
...
def test_validation_count():
    ...
    mods = list_modules()
    assert len(mods['validation']) > 0, "Module has no theorems"
...
def test_validation_result_format():
    ...
    for name in _MODULE_MAP.get('validation', []):
        r = REGISTRY[name]()
